...
1. Copy `settings.docker.php` to your Drupal's site folder, usually `sites/default/`. 
1. Run `make up` to start the environment.
1. Run `make build` to build the codebase.

Note: Do not edit `docker-compose.yml`. If you wish to customize a value in it, then copy from there to
`docker-compose.override.yml` and change the value, which will override it.
